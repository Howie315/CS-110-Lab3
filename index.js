  ...
  if (!document.querySelector("#pauseTweets").checked) {
    fetch("http://50.21.190.71/get_tweets")
      ...
      .then((data) => {
       ...
       const tweetsWithoutDuplicates = tweets.filter((value, index, self) => 
...
document.getElementById('search-input').addEventListener('input', function(e) {
  ...
  let filtered = tweets.filter(tweet => 
    tweet.text.includes(searchWords) || tweet.date === searchWords || tweet.user_name.includes(searchWords)
);
